...
export default function ChatBot({ currentUserId }) {
  ...
  const { data: customerProfile, isLoading: profileLoading, error: profileError } = useGetProfileQuery();
  ...
  // Extract customer data - use API data if available, otherwise use token data
  const customer = customerProfile?.data || tokenData;
      ...
      <div className="flex items-center gap-2 sm:gap-3 md:gap-2 px-3 py-2 sm:px-4 sm:py-3 md:px-6 md:py-4 bg-card  border-b border-border  shadow-sm shrink-0">
        {profileLoading && !customer ? (
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gray-300 dark:bg-slate-600 rounded-full animate-pulse"></div>
            <div className="space-y-2">
              <div className="h-4 w-32 bg-gray-300 dark:bg-slate-600 rounded animate-pulse"></div>
              <div className="h-3 w-20 bg-gray-300 dark:bg-slate-600 rounded animate-pulse"></div>
            </div>
          </div>
        ) : (
          <>
            ...
            >
              {(customer?.name || customer?.email)?.charAt(0)?.toUpperCase() || 'C'}
            </Avatar>
