...
const AgentDetailCard = ({ agentData, isDark, onDownloadPDF, handleDownloadExcel }) => {
  const [expanded, setExpanded] = useState(false);

  const agent = agentData.agent;
  
  // Use pre-calculated stats from API
  const queryStats = agentData.queries;
  const ticketStats = agentData.tickets; // Old Ticket model (chat tickets)
  const emailTicketStats = agentData.emailTickets; // Email Tickets (same as Agent Dashboard)
  const combinedStats = agentData.combined;
  ...
  // Email Ticket metrics (showing Email Tickets - same as Agent Dashboard TicketStats)
  const agentTickets = {
  ...
  const escalationRate = agentTickets.total > 0 
    ? Math.round((agentTickets.escalated / agentTickets.total) * 100)
    : 0;

  // Status distribution data for chart (Email Ticket statuses)
  const statusDistribution = [
    { name: 'Closed', value: agentTickets.resolved, fill: '#10b981' },
    { name: 'Open', value: agentTickets.open, fill: '#3b82f6' },
    { name: 'Pending', value: agentTickets.inProgress, fill: '#f59e0b' },
    { name: 'Escalated', value: agentTickets.escalated, fill: '#ef4444' },
  ].filter((item) => item.value > 0);

  // Prepare Radar Chart Data for Queries
  const maxQueryValue = Math.max(
    agentQueries.total,
    agentQueries.resolved,
    agentQueries.accepted,
    agentQueries.inProgress,
    agentQueries.escalated,
    10
  );
  const queryRadarData = [
    {
      metric: `Total (${agentQueries.total})`,
      value: agentQueries.total,
      actualValue: agentQueries.total,
      fullMark: maxQueryValue,
    },
    {
      metric: `Resolved (${agentQueries.resolved})`,
      value: agentQueries.resolved,
      actualValue: agentQueries.resolved,
      fullMark: maxQueryValue,
    },
    {
      metric: `Accepted (${agentQueries.accepted})`,
      value: agentQueries.accepted,
      actualValue: agentQueries.accepted,
      fullMark: maxQueryValue,
    },
    {
      metric: `In Progress (${agentQueries.inProgress})`,
      value: agentQueries.inProgress,
      actualValue: agentQueries.inProgress,
      fullMark: maxQueryValue,
    },
    {
      metric: `Escalated (${agentQueries.escalated})`,
      value: agentQueries.escalated,
      actualValue: agentQueries.escalated,
      fullMark: maxQueryValue,
    },
  ];
  ...
  const maxTicketValue = Math.max(
    agentTickets.total,
    agentTickets.resolved,
    agentTickets.open,
    agentTickets.inProgress,
    agentTickets.escalated,
    10
  );
  const ticketRadarData = [
    {
      metric: `Total (${agentTickets.total})`,
      value: agentTickets.total,
      actualValue: agentTickets.total,
      fullMark: maxTicketValue,
    },
    {
      metric: `Closed (${agentTickets.resolved})`,
      value: agentTickets.resolved,
      actualValue: agentTickets.resolved,
      fullMark: maxTicketValue,
    },
    {
      metric: `Open (${agentTickets.open})`,
      value: agentTickets.open,
      actualValue: agentTickets.open,
      fullMark: maxTicketValue,
    },
    {
      metric: `Pending (${agentTickets.inProgress})`,
      value: agentTickets.inProgress,
      actualValue: agentTickets.inProgress,
      fullMark: maxTicketValue,
    },
    {
      metric: `Escalated (${agentTickets.escalated})`,
      value: agentTickets.escalated,
      actualValue: agentTickets.escalated,
      fullMark: maxTicketValue,
    },
  ];
    ...
    <div className="mb-4 border border-border  rounded-lg overflow-hidden bg-card  shadow-sm">
      ...
      {expanded && (
        <div className="border-t border-border  p-6 bg-muted/50 /50">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-6">
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => {
                  if (e.target.value) {
                    onDownloadPDF(agentData, e.target.value);
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => {
                  if (e.target.value) {
                    handleDownloadExcel(agentData, e.target.value);
                    e.target.value = ''; // Reset
                  }
                }}
                defaultValue=""
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-800 dark:to-slate-700 rounded-lg border-2 border-primary/20 dark:border-blue-700 p-4 mb-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
              ...
              <div className="bg-card  rounded-lg p-3 border border-blue-100 dark:border-slate-600">
                ...
                <p className="text-2xl font-bold text-foreground ">{agentQueries.accepted}</p>
              </div>
          ...
          {/* Email Ticket Statistics (same as Agent Dashboard) */}
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-slate-800 dark:to-slate-700 rounded-lg border-2 border-green-200 dark:border-green-700 p-4 mb-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              ...
              <div className="bg-card  rounded-lg p-3 border border-blue-100 dark:border-slate-600">
                ...
                <p className="text-2xl font-bold text-foreground ">{agentTickets.open}</p>
              </div>
              <div className="bg-card  rounded-lg p-3 border border-amber-100 dark:border-slate-600">
...
const AgentPerformanceDetail = () => {
  ...
  // Performance data is already pre-calculated per agent
  const agentPerformanceList = useMemo(() => {
    if (!performanceData?.data) return [];
    console.log('📊 Agent Performance Data:', performanceData.data);
    return performanceData.data;
  }, [performanceData]);

  const filteredAgents = useMemo(() => {
    let filtered = agentPerformanceList.filter(
      (item) =>
    ...
    return filtered;
  }, [agentPerformanceList, searchTerm, sortBy]);
  ...
  const handleDownloadAllAgentsPDF = async (period) => {
    ...
    try {
      ...
      // Generate report for all agents using pre-calculated stats
      let allAgentsHTML = '';

      agentPerformanceList.forEach((agentData, index) => {
        const agent = agentData.agent;
        const queryStats = agentData.queries;
        const emailTicketStats = agentData.emailTickets; // Use EmailTickets instead of old Tickets
        const combinedStats = agentData.combined;
        ...
        allAgentsHTML += `
          ...
          ${index < agentPerformanceList.length - 1 ? '<div style="page-break-after: always;"></div>' : ''}
        `;
      });
      ...
      const fullHTML = `
        ...
        <body>
          <div class="header">
            ...
            <p><strong>Total Agents:</strong> ${agentPerformanceList.length}</p>
          </div>
          ...
          <div class="summary">
            ...
            <div class="summary-grid">
              <div class="summary-card">
                ...
                <div class="value">${agentPerformanceList.length}</div>
              </div>
              <div class="summary-card">
                <div class="label">Active Agents</div>
                <div class="value">${agentPerformanceList.filter((item) => item.agent.isActive).length}</div>
              </div>
              <div class="summary-card">
                <div class="label">Total Tickets</div>
                <div class="value">${agentPerformanceList.reduce((sum, item) => sum + item.tickets.total, 0)}</div>
              </div>
              <div class="summary-card">
      ...
      setTimeout(() => {
        ...
        toast.success(`All agents ${period} PDF report generated successfully!`, { autoClose: 3000 });
      }, 500);
    } catch (error) {
  ...
  const handleDownloadAllAgentsExcel = async (period) => {
    const toastId = toast.loading(`Generating ${period} Excel report for all agents...`);
    ...
    try {
      ...
      const excelData = agentPerformanceList.map((agentData, index) => {
        const agent = agentData.agent;
        const queryStats = agentData.queries;
        const emailTicketStats = agentData.emailTickets;
        const combinedStats = agentData.combined;
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Agents Performance');

      // Generate file - use writeFile (browser-compatible)
      XLSX.writeFile(
        wb,
        `All_Agents_Performance_${period}_${format(new Date(), 'yyyy-MM-dd')}.xlsx`,
        { bookType: 'xlsx', type: 'binary' }
      );

      toast.dismiss(toastId);
      toast.success(`All agents ${period} Excel report downloaded successfully!`, { autoClose: 3000 });
    } catch (error) {
      console.error('Error generating all agents Excel:', error);
      toast.dismiss(toastId);
      toast.error('Failed to generate Excel report');
    }
  };

  const handleDownloadExcel = async (agentData, period) => {
    const agent = agentData.agent;
    ...
    try {
      ...
      // Use pre-calculated stats from agentData
      const queryStats = agentData.queries;
      const emailTicketStats = agentData.emailTickets;
      const combinedStats = agentData.combined;
      ...
      const excelData = [
        {
          'Report Type': 'Agent Performance Summary',
          'Agent Name': agent.name,
      ...
      console.error('Error generating agent Excel:', error);
      toast.dismiss(toastId);
      toast.error('Failed to generate Excel report');
    }
  };

  const handleDownloadPDF = async (agentData, period) => {
    const agent = agentData.agent;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-purple-700 dark:text-purple-400">
                Generate comprehensive PDF reports for all {agentPerformanceList.length} agents
              </p>
            </div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (value) {
                      handleDownloadAllAgentsPDF(value);
                      setTimeout(() => {
                        e.target.value = '';
                      }, 100);
                    }
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => {
                    const value = e.target.value;
                    if (value) {
                      handleDownloadAllAgentsExcel(value);
                      setTimeout(() => {
                        e.target.value = '';
                      }, 100);
                    }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        <div className="p-6 rounded-lg bg-card  border border-border ">
          ...
          <h3 className="text-3xl font-bold text-foreground  mt-1">
            {agentPerformanceList.length}
          </h3>
        </div>
        <div className="p-6 rounded-lg bg-card  border border-border ">
          <p className="text-sm text-muted-foreground ">Currently Active</p>
          <h3 className="text-3xl font-bold text-green-600  mt-1">
            {agentPerformanceList.filter((item) => item.agent.isActive).length}
          </h3>
        </div>
        <div className="p-6 rounded-lg bg-card  border border-border ">
