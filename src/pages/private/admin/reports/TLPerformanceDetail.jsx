...
const TLDetailCard = ({ TLData, isDark, onDownloadPDF, handleDownloadExcel }) => {
  const [expanded, setExpanded] = useState(false);

  const agent = TLData.agent;
  
  // Use pre-calculated stats from API
  const queryStats = TLData.queries;
  const ticketStats = TLData.tickets; // Old Ticket model (chat tickets)
  const emailTicketStats = TLData.emailTickets; // Email Tickets (same as TL Dashboard)
  const combinedStats = TLData.combined;
  ...
  // Email Ticket metrics (showing Email Tickets - same as TL Dashboard TicketStats)
  const TLTickets = {
  ...
  const escalationRate = TLTickets.total > 0 
    ? Math.round((TLTickets.escalated / TLTickets.total) * 100)
    : 0;

  // Status distribution data for chart (Email Ticket statuses)
  const statusDistribution = [
    { name: 'Closed', value: TLTickets.resolved, fill: '#10b981' },
    { name: 'Open', value: TLTickets.open, fill: '#3b82f6' },
    { name: 'Pending', value: TLTickets.inProgress, fill: '#f59e0b' },
    { name: 'Escalated', value: TLTickets.escalated, fill: '#ef4444' },
  ...
  const maxQueryValue = Math.max(
    ...
    TLQueries.resolved,
    TLQueries.accepted,
    TLQueries.inProgress,
    TLQueries.escalated,
    10
  );
  const queryRadarData = [
    {
      metric: `Total (${TLQueries.total})`,
      value: TLQueries.total,
      actualValue: TLQueries.total,
      fullMark: maxQueryValue,
    },
    {
      metric: `Resolved (${TLQueries.resolved})`,
      value: TLQueries.resolved,
      actualValue: TLQueries.resolved,
      fullMark: maxQueryValue,
    },
    {
      metric: `Accepted (${TLQueries.accepted})`,
      value: TLQueries.accepted,
      actualValue: TLQueries.accepted,
      fullMark: maxQueryValue,
    },
    {
      metric: `In Progress (${TLQueries.inProgress})`,
      value: TLQueries.inProgress,
      actualValue: TLQueries.inProgress,
      fullMark: maxQueryValue,
    },
    {
      metric: `Escalated (${TLQueries.escalated})`,
      value: TLQueries.escalated,
      actualValue: TLQueries.escalated,
      fullMark: maxQueryValue,
    },
  ];
  ...
  const maxTicketValue = Math.max(
    ...
    TLTickets.resolved,
    TLTickets.open,
    TLTickets.inProgress,
    TLTickets.escalated,
    10
  );
  const ticketRadarData = [
    {
      metric: `Total (${TLTickets.total})`,
      value: TLTickets.total,
      actualValue: TLTickets.total,
      fullMark: maxTicketValue,
    },
    {
      metric: `Closed (${TLTickets.resolved})`,
      value: TLTickets.resolved,
      actualValue: TLTickets.resolved,
      fullMark: maxTicketValue,
    },
    {
      metric: `Open (${TLTickets.open})`,
      value: TLTickets.open,
      actualValue: TLTickets.open,
      fullMark: maxTicketValue,
    },
    {
      metric: `Pending (${TLTickets.inProgress})`,
      value: TLTickets.inProgress,
      actualValue: TLTickets.inProgress,
      fullMark: maxTicketValue,
    },
    {
      metric: `Escalated (${TLTickets.escalated})`,
      value: TLTickets.escalated,
      actualValue: TLTickets.escalated,
      fullMark: maxTicketValue,
    },
  ];
    ...
    <div className="mb-4 border border-border  rounded-lg overflow-hidden bg-card  shadow-sm">
      ...
      {expanded && (
        <div className="border-t border-border  p-6 bg-muted/50 /50">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-6">
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => {
                  if (e.target.value) {
                    onDownloadPDF(TLData, e.target.value);
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => {
                  if (e.target.value) {
                    handleDownloadExcel(TLData, e.target.value);
                    e.target.value = ''; // Reset
                  }
                }}
                defaultValue=""
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-800 dark:to-slate-700 rounded-lg border-2 border-primary/20 dark:border-blue-700 p-4 mb-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
              ...
              <div className="bg-card  rounded-lg p-3 border border-blue-100 dark:border-slate-600">
                ...
                <p className="text-2xl font-bold text-foreground ">{TLQueries.accepted}</p>
              </div>
          ...
          {/* Email Ticket Statistics (same as TL Dashboard) */}
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-slate-800 dark:to-slate-700 rounded-lg border-2 border-green-200 dark:border-green-700 p-4 mb-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              ...
              <div className="bg-card  rounded-lg p-3 border border-blue-100 dark:border-slate-600">
                ...
                <p className="text-2xl font-bold text-foreground ">{TLTickets.open}</p>
              </div>
              <div className="bg-card  rounded-lg p-3 border border-amber-100 dark:border-slate-600">
...
const TLPerformanceDetail = () => {
  ...
  // Performance data is already pre-calculated per TL
  const TLPerformanceList = useMemo(() => {
    if (!performanceData?.data) return [];
    console.log('📊 TL Performance Data:', performanceData.data);
    return performanceData.data;
  }, [performanceData]);

  const filteredTLs = useMemo(() => {
    let filtered = TLPerformanceList.filter(
      (item) =>
    ...
    return filtered;
  }, [TLPerformanceList, searchTerm, sortBy]);

  const handleDownloadAllTLsPDF = async (period) => {
    const toastId = toast.loading(`Generating ${period} report for all TLs...`);
    ...
    try {
      ...
      // Generate report for all TLs using pre-calculated stats
      let allTLsHTML = '';

      TLPerformanceList.forEach((TLData, index) => {
        const agent = TLData.agent;
        const queryStats = TLData.queries;
        const emailTicketStats = TLData.emailTickets; // Use EmailTickets instead of old Tickets
        const combinedStats = TLData.combined;
        ...
        allTLsHTML += `
          ...
          ${index < TLPerformanceList.length - 1 ? '<div style="page-break-after: always;"></div>' : ''}
        `;
      });
      ...
      const fullHTML = `
        ...
        <head>
          <meta charset="utf-8">
          <title>all TLs Performance Report - ${period}</title>
          <style>
          ...
          <div class="summary">
            ...
            <div class="summary-grid">
              <div class="summary-card">
                <div class="label">Total TLs</div>
                <div class="value">${TLPerformanceList.length}</div>
              </div>
              <div class="summary-card">
                <div class="label">Active TLs</div>
                <div class="value">${TLPerformanceList.filter((item) => item.agent.isActive).length}</div>
              </div>
              <div class="summary-card">
                <div class="label">Total Tickets</div>
                <div class="value">${TLPerformanceList.reduce((sum, item) => sum + item.tickets.total, 0)}</div>
              </div>
              <div class="summary-card">
      ...
      setTimeout(() => {
        ...
        toast.success(`all TLs ${period} PDF report generated successfully!`, { autoClose: 3000 });
      }, 500);
    } catch (error) {
      console.error('Error generating all TLs PDF:', error);
      toast.dismiss(toastId);
      toast.error('Failed to generate PDF report');
    }
  };

  const handleDownloadAllTLsExcel = async (period) => {
    const toastId = toast.loading(`Generating ${period} Excel report for all TLs...`);
    ...
    try {
      ...
      const excelData = TLPerformanceList.map((TLData, index) => {
        const agent = TLData.agent;
        const queryStats = TLData.queries;
        const emailTicketStats = TLData.emailTickets;
        const combinedStats = TLData.combined;
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'TLs Performance');

      // Generate file - use writeFile (browser-compatible)
      XLSX.writeFile(
        wb,
        `All_TLs_Performance_${period}_${format(new Date(), 'yyyy-MM-dd')}.xlsx`,
        { bookType: 'xlsx', type: 'binary' }
      );

      toast.dismiss(toastId);
      toast.success(`all TLs ${period} Excel report downloaded successfully!`, { autoClose: 3000 });
    } catch (error) {
      console.error('Error generating all TLs Excel:', error);
      toast.dismiss(toastId);
      toast.error('Failed to generate Excel report');
    }
  };

  const handleDownloadExcel = async (TLData, period) => {
    const agent = TLData.agent;
    ...
    try {
      ...
      // Use pre-calculated stats from TLData
      const queryStats = TLData.queries;
      const emailTicketStats = TLData.emailTickets;
      const combinedStats = TLData.combined;
      ...
      const excelData = [
        {
          'Report Type': 'TL Performance Summary',
          'TL Name': agent.name,
          'TL Email': agent.email,
      ...
      console.error('Error generating TL Excel:', error);
      toast.dismiss(toastId);
      toast.error('Failed to generate Excel report');
    }
  };

  const handleDownloadPDF = async (TLData, period) => {
    const agent = TLData.agent;
    ...
    try {
      ...
      const pdfHTML = `
        ...
        <head>
          <meta charset="utf-8">
          <title>TL Performance Report - ${period}</title>
          <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .TL-info { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 25px; border-radius: 10px; margin-bottom: 30px; }
            .TL-info h2 { font-size: 28px; margin-bottom: 10px; }
            .TL-info p { font-size: 16px; opacity: 0.95; }
    ...
    <div className="p-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-bold text-purple-900 dark:text-purple-300 mb-1">
                📥 Download all TLs Performance Reports
              </h3>
              <p className="text-sm text-purple-700 dark:text-purple-400">
                Generate comprehensive PDF reports for all {TLPerformanceList.length} TLs
              </p>
            </div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => {
                    const value = e.target.value;
                    if (value) {
                      handleDownloadAllTLsExcel(value);
                      setTimeout(() => {
                        e.target.value = '';
                      }, 100);
                    }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        <div className="p-6 rounded-lg bg-card  border border-border ">
          <p className="text-sm text-muted-foreground ">Total TLs</p>
          <h3 className="text-3xl font-bold text-foreground  mt-1">
            {TLPerformanceList.length}
          </h3>
        </div>
        <div className="p-6 rounded-lg bg-card  border border-border ">
          <p className="text-sm text-muted-foreground ">Currently Active</p>
          <h3 className="text-3xl font-bold text-green-600  mt-1">
            {TLPerformanceList.filter((item) => item.agent.isActive).length}
          </h3>
        </div>
        <div className="p-6 rounded-lg bg-card  border border-border ">
      ...
      {filteredTLs.length === 0 && (
        <div className="text-center py-12 text-muted-foreground ">
          No TLs found matching your search.
        </div>
      )}
    </div>
  );
